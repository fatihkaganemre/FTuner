...
struct TuneProvider {
    ...
    static let notesAndFrequencies: [String: Double] = [
    ...
    static let minFrequency = 15.0
    static let maxFrequency = 7903.0
    
    static func getTuneAndDifference(fromFrequency frequency: Double) -> (tune: String, difference: Double)? {
        ...
        let tuneAndDifference = tuneAndDifferences
            .min { lhs, rhs in abs(lhs.value) < abs(rhs.value) }
            .map { (tune: $0.key, difference: $0.value) }
        
        return tuneAndDifference
    }
    
    static func calculateTheDifferencePercentage(ofFrequency frequency: Double) -> Int? {
